#include "BordesChoca.h"
#include "glut.h"

BordesChoca::BordesChoca(float x1, float y1, float x2, float y2) {
    limite1.x = x1;
    limite1.y = y1;
...
BordesChoca::BordesChoca(ETSIDI::Vector2D l1, ETSIDI::Vector2D l2) {
    limite1 = l1;
    limite2 = l2;
}
void BordesChoca::Dibuja() {
    glDisable(GL_LIGHTING);
    glColor3ub(200, 200, 200);
    glBegin(GL_POLYGON);
    glVertex3d(limite1.x-0.5, limite1.y, 0);
    glVertex3d(limite1.x-0.5, limite2.y, 0);
    ...
    glVertex3d(limite2.x+0.5, limite2.y, 0);
    glVertex3d(limite2.x+0.5, limite1.y, 0);
    glEnd();
    glEnable(GL_LIGHTING);
}
float BordesChoca::distancia(ETSIDI::Vector2D punto, ETSIDI::Vector2D* direccion) {
